...
char		*fontnames[2] =
{
	...
	"/lib/font/bit/lucm/unicode.14.font"
};
...
iconinit(void)
{
	...
	if(tagcols[BACK] == nil) {
	/* Dark Theme */
tagcols[BACK] = display->black;                       // Set background to black
tagcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DRed); 
tagcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xD3D3D3FF);  // Border color (dark blue)
tagcols[TEXT] = display->white;                       // Text color to white
tagcols[HTEXT] = display->white;                      // Highlighted text to white

textcols[BACK] = display->black;                      // Text area background to black
textcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DRed);  // Highlight color (dark grey)
textcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xD3D3D3FF);  // Border color (dark blue)
textcols[TEXT] = display->white;                      // Text color to white
textcols[HTEXT] = display->white;                     // Highlighted text to white
	...
	tmp = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DRed);
	draw(modbutton, r, tmp, nil, ZP);
	freeimage(tmp);

	r = button->r;
	colbutton = allocimage(display, r, screen->chan, 1, 0xD3D3D3FF);
